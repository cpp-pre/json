...
BOOST_AUTO_TEST_CASE (function_traits_test_lambda) {
    ...
    BOOST_TEST_REQUIRE(my_lambda_types::arity == 4 , " lambda arity is wrong");
    BOOST_TEST_REQUIRE(is_same, " return type wrong");

    is_same =  std::is_same<my_lambda_types::arg<0>, bool>::value;
    BOOST_TEST_REQUIRE(is_same, " argument ist not as expected");
    ...
    BOOST_TEST_REQUIRE(is_same, " function type  ist not as expected");

    auto std_function = pre::functional::to_std_function(my_lambda);

    BOOST_TEST_REQUIRE(std_function(true, int{42}, double{3.14}, std::string{"Hello World"}) == 23 , " call to std::funtion is wrong");
}
...
BOOST_AUTO_TEST_CASE (function_traits_test_lambda_copy_captures) {
    ...
    is_same =  std::is_same<my_lambda_types::arg<0>, bool>::value;
    BOOST_TEST_REQUIRE(is_same, " argument 0 ist not as expected " );

    is_same =  std::is_same<my_lambda_types::arg<1>, int>::value;
    BOOST_TEST_REQUIRE(is_same, " argument 1 ist not as expected");

    is_same =  std::is_same<my_lambda_types::arg<2>, double>::value;
    BOOST_TEST_REQUIRE(is_same, " argument 2 ist not as expected");

    is_same =  std::is_same<my_lambda_types::arg<3>, std::string>::value;
    BOOST_TEST_REQUIRE(is_same, " argument 3 ist not as expected");

    typedef std::function<int(bool, int, double, std::string)> expected_function_type;
    is_same = std::is_same<my_lambda_types::function_type , expected_function_type>::value;
    BOOST_TEST_REQUIRE(!is_same, " function type  ist not as expected with capture it is different");
    auto std_function = pre::functional::to_std_function(my_lambda_copy_capture);

    BOOST_TEST_REQUIRE(std_function(true, int{42}, double{3.14}, std::string{"Hello World"}) == (42+12.3) , " call to std::funtion is wrong");
}
...
BOOST_AUTO_TEST_CASE (function_traits_test_freestandingfunctions) {
  ...
  typedef pre::type_traits::function_traits<decltype(function_to_instrospect)> introspector;
    ...
    BOOST_TEST_REQUIRE(introspector::arity == 2 , " function arity is wrong");
    BOOST_TEST_REQUIRE(
    ...
    BOOST_TEST_REQUIRE(std_function(std::string{"Hello World"}, 21.00) == 42 , " call to std::funtion is wrong");
}
...
BOOST_AUTO_TEST_CASE (function_traits_test_memberfunctions) {

  {
    typedef pre::type_traits::function_traits<decltype(&some_class::member_to_introspect)> introspector;
      ...
      BOOST_TEST_REQUIRE(introspector::arity == 3 , " function arity is wrong");
      BOOST_TEST_REQUIRE(
        (std::is_same<introspector::result_type, int>::value),
        " return type wasn't correclty extracted");
     ...
     BOOST_TEST_REQUIRE(
      ...
      BOOST_TEST_REQUIRE(std_function(instance, std::string{"Hello World"}, 21.00) == 42 , " call to std::funtion is wrong");
  }
